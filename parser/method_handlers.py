...
def execute_g1(params, machine):
	...
	for param in params:
		if param[0] is 'X':
			X += float(param[1:])
			message = message + 'Move ' + str(param[1:]) + str(machine.selected_units[0]) + ' X. '
		elif param[0] is 'Y':
			Y += float(param[1:])
			message = message + 'Move ' + str(param[1:]) + str(machine.selected_units[0]) + ' Y. '
		elif param[0] is 'Z':
			Z += float(param[1:])
			message = message + 'Move ' + str(param[1:]) + str(machine.selected_units[0]) + ' Z. '
		elif param[0] is 'E':
			message = message + 'Extrude '+ str(param[1:]) +' of filament. '
		elif param[0] is 'F':
			machine.set_feed_rate(float(param[1:]))
			message = message + 'Set feed rate to ' + str(param[1:]) + str(machine.selected_units[0]) + '/minute. '
...
def execute_m117(params, machine):
	return '==> Displaying message to LCD'
